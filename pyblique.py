...
def best_ap_split(data, attr, metric):
    ...
    split_evals = {}
    for s in possible_splits(data, attr):
        cond = data[:, attr] <= s
...
def get_split_vector(data, metric):
    ...
    result = [best_ap_split(data, i, metric) for i in range(n_attrs)]
